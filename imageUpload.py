...
def upload_person_image(imagePath,imageName,personName):
	...
	res=cloudinary.uploader.upload(imagePath, public_id = 'SOLAMS/dataset/'+personName+'/'+imageName)
...
def upload_image(imagePath,imageName):
	cloudinary_config()
	imageName=os.path.splitext(imageName)[0]
	res=cloudinary.uploader.upload(imagePath, public_id = 'SOLAMS/tmp/'+imageName)
	#print res
	#return response
